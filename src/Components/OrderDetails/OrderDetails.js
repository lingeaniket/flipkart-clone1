...
const OrderDetails = ({ method }) => {
    ...
    useEffect(() => {
        ...
        if (mt === "postCheckout") {
            ...
            setSelectedOrder(() => orders.filter((item) => item.order_id === order_id)[0]);

            setStatus_id(orders.filter((item) => item.order_id === order_id)[0].order_status_index);

            setStatus(orders.filter((item) => item.order_id === order_id)[0].order_status);

            setSelectedProduct(
                orders
                    .filter((item) => item.order_id === order_id)[0]
                    .order_details.products.filter((product) => product.unit_id === unit_id)[0].unit
            );
        }
