    ...
    return (
        ...
        >
            <div className="_order_002 w-1-1">
                    ...
                    <div className="_order_002 w-1-1">
                        <div className="flexCenCen w-1-3">
                            <div className="_order_078">
                                <img className="_order_079" src={unit.unit.thumbnail} alt="" />
                            </div>
                        </div>
                        <div className="w-2-3">
                            <div style={{ padding: "0 10px" }}>
                                ...
                                {type === "list_item" && order.order_details.products.length - 1 > 0 && (
                                    ...
                                    >
                                        + {order.order_details.products.length - 1} More Item
                                        {order.order_details.products.length - 1 === 1 ? "" : "s"}
                                    </span>
                                )}
                            </div>
                        </div>
                ...
                <div className="w-1-5">
                    {type === "list_item" && <div className="_order_082">Total Order $ {order.order_details.price_details.price}</div>}
                </div>
                <div className="_order_083 w-2-5">
                    <div>
                        <div
                            className={`_order_084 _order_016 ${order.order_status !== "cancelled" ? "_order_046" : "_order_cancelled"}`}
                        ></div>
                        <span className="_order_085">
                        ...
                        <div
                            className="_order_086"
                            style={{
                                textTransform: "capitalize",
                            }}
                        >
                            Your item is {status === "nearest_hub" && "reached"} {status.replace("_", " ")} on{" "}
                            {status === "confirmed" ? formattedFullDate(order.order_date) : formattedFullDate(order.order_timeline[status])}
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div className="_order_091">
                        <ul>
                            <li className="_order_092">
                                <span>
                                    {order.order_details.price_details.price} has been refunded on{" "}
                                    {formattedDate(order.order_timeline.cancelled)}{" "}
                                </span>
                            </li>
                        </ul>
                    </div>
